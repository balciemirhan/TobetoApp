...
class ExamBloc extends Bloc<ExamEvent, ExamState> {
  final ExamRepository _examRepostiory;

  ExamBloc(this._examRepostiory) : super(ExamInitial()) {
    on<GetExam>(_onGetExam);
  }

  Future<void> _onGetExam(GetExam event, Emitter<ExamState> emit) async {
    emit(ExamLoading());

    try {
      final List<Exam> exam = await _examRepostiory.getExam();
